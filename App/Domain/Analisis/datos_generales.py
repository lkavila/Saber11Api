...
def total_registros_niveles_desempeno_por_periodo(periodo, colegio=None):
    dataframe = get_dataframe_for_year(periodo)
    if colegio is not None:
        dataframe = mantener_columnas(dataframe, (puntajes + desempenos).append('COLE_NOMBRE_ESTABLECIMIENTO'))
        dataframe = dataframe[dataframe['COLE_NOMBRE_ESTABLECIMIENTO'] == colegio.upper()]
    else:
        dataframe = mantener_columnas(dataframe, (puntajes + desempenos))

    toal_estudiantes = dataframe.shape[0]
    resultado = {}
    datos_periodo = {'Total_estudiantes': toal_estudiantes}

    for puntaje in puntajes:
        datos_periodo['Promedio '+puntaje] = round(numpy.mean(dataframe[puntaje].values), 2)

    for desempeno in desempenos:
        datos_periodo['Porcentaje '+desempeno] = dataframe[desempeno].value_counts().apply(lambda x: round((x/toal_estudiantes)*100),1).to_dict()
